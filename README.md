...
| Native Texture2D and Sprite object files            | Image                                            | Texture2D → PNG/DDS; Sprite → PNG      | One-way conversion; a Sprite stores no pixels, so change its look by editing the Texture2D it resolves to — see [Editing a Sprite](cmd/README.md#editing-a-sprite) |
...
| 原生 Texture2D、Sprite 对象文件            | 图片                             | Texture2D → PNG/DDS；Sprite → PNG | 单向转换；Sprite 本身不存像素，改外观请修改它所指向的 Texture2D，详见 [修改 Sprite](cmd/README.md#修改-sprite) |
...
| native Texture2D、Sprite object file            | 画像                                         | Texture2D → PNG/DDS、Sprite → PNG | 一方向変換。Sprite 自体はピクセルを持たないため、見た目を変えるには参照先の Texture2D を編集します。[Sprite の編集](cmd/README.md#sprite-の編集)を参照 |
