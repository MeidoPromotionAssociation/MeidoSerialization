...
| `.dbconf`                                           | Physics and collider payload                     | Native ↔ JSON                          | KCES ExportCM reuses this extension for a COM3D2.5-readable JSON export; that game-generated intermediate is not a KCES resource and is rejected                                                                                             |
| `.dbcol`                                            | Physics and collider payload                     | Native ↔ JSON                          | KCES ExportCM reuses this extension for a COM3D2.5-readable JSON export; that game-generated intermediate is not a KCES resource and is rejected                                                                                             |
...
| `.dslcol`                                           | Physics and collider payload                     | Native ↔ JSON                          | KCES ExportCM reuses this extension for a COM3D2.5-readable JSON export; that game-generated intermediate is not a KCES resource and is rejected                                                                                             |
...
| `.undressdat`                                       | Underwear peel setup                             | Native ↔ editing JSON                  | The native format is JSON; KCES2 only, and usable only paired with the `.undresspdat` of the same name                                                                                                                                       |
| `.undresspdat`                                      | Underwear peel precomputed cache                 | Native ↔ editing JSON                  | The native format is JSON; KCES2 only, derived from the same-named `.undressdat` and usable only paired with it                                                                                                                              |
...
| Native Texture2D and Sprite object files            | Image                                            | Texture2D → PNG/DDS; Sprite → PNG      | One-way conversion; a Sprite stores no pixels, so change its look by editing the Texture2D it resolves to — see [Editing a Sprite](docs/cli-document.md#editing-a-sprite)                                                                    |
...
Read the [complete CLI documentation](docs/cli-document.md).
...
is in [docs/cli-document.md](docs/cli-document.md). `--strict` (`-s`) enables content-based type determination; `--type` (`-t`)
...
| `.dbconf`                                  | 物理与碰撞载荷                   | 原生格式 ↔ JSON                   | KCES ExportCM 会用同一扩展名写出供 COM3D2.5 读取的 JSON 导出物；那是游戏生成的中间体而非 KCES 资源，会被拒绝                                                                                            |
| `.dbcol`                                   | 物理与碰撞载荷                   | 原生格式 ↔ JSON                   | KCES ExportCM 会用同一扩展名写出供 COM3D2.5 读取的 JSON 导出物；那是游戏生成的中间体而非 KCES 资源，会被拒绝                                                                                            |
...
| `.dslcol`                                  | 物理与碰撞载荷                   | 原生格式 ↔ JSON                   | KCES ExportCM 会用同一扩展名写出供 COM3D2.5 读取的 JSON 导出物；那是游戏生成的中间体而非 KCES 资源，会被拒绝                                                                                            |
| `.ikcol`                                   | 物理与碰撞载荷                   | 原生格式 ↔ JSON                   |                                                                                                                                                                     |
| `.ikcol.bytes`                             | 物理与碰撞载荷                   | 原生格式 ↔ JSON                   |                                                                                                                                                                     |
| `.limbcol`                                 | 物理与碰撞载荷                   | 原生格式 ↔ JSON                   |                                                                                                                                                                     |
| `.hitcheck`                                | 碰撞检测数据                     | 原生格式 ↔ editing JSON           |                                                                                                                                                                     |
| `.undressdat`                              | 内衣扒开设置数据                 | 原生格式 ↔ editing JSON           | 原生格式本身就是 JSON；仅 KCES2 有，必须与同名 `.undresspdat` 成对使用                                                                                              |
| `.undresspdat`                             | 内衣扒开预计算缓存               | 原生格式 ↔ editing JSON           | 原生格式本身就是 JSON；仅 KCES2 有，由同名 `.undressdat` 烘焙而来                                                                                                   |
...
| 原生 Texture2D、Sprite 对象文件            | 图片                             | Texture2D → PNG/DDS；Sprite → PNG | 单向转换；Sprite 本身不存像素，改外观请修改它所指向的 Texture2D，详见 [修改 Sprite](docs/cli-document.md#修改-sprite)                                               |
...
查看 CLI 使用说明：[说明](https://github.com/MeidoPromotionAssociation/MeidoSerialization/blob/main/docs/cli-document.md)
...
[docs/cli-document.md](docs/cli-document.md)。`--strict`（`-s`）启用基于内容的类型判断；`--type`（`-t`）按原生 类型或 `<type>.json` 过滤目录操作。
...
| `.dbconf`                                       | 物理・コライダー payload                     | native ↔ JSON                     | KCES ExportCM は同じ拡張子で COM3D2.5 が読む JSON エクスポートを書き出しますが、それはゲームが生成した中間物であり KCES リソースではないため拒否されます                                                                                                                          |
| `.dbcol`                                        | 物理・コライダー payload                     | native ↔ JSON                     | KCES ExportCM は同じ拡張子で COM3D2.5 が読む JSON エクスポートを書き出しますが、それはゲームが生成した中間物であり KCES リソースではないため拒否されます                                                                                                                          |
...
| `.dslcol`                                       | 物理・コライダー payload                     | native ↔ JSON                     | KCES ExportCM は同じ拡張子で COM3D2.5 が読む JSON エクスポートを書き出しますが、それはゲームが生成した中間物であり KCES リソースではないため拒否されます                                                                                                                          |
| `.ikcol`                                        | 物理・コライダー payload                     | native ↔ JSON                     |                                                                                                                                                                                                                         |
| `.ikcol.bytes`                                  | 物理・コライダー payload                     | native ↔ JSON                     |                                                                                                                                                                                                                         |
| `.limbcol`                                      | 物理・コライダー payload                     | native ↔ JSON                     |                                                                                                                                                                                                                         |
| `.hitcheck`                                     | hit-check data                               | native ↔ editing JSON             |                                                                                                                                                                                                                         |
| `.undressdat`                                   | 下着めくり設定データ                         | native ↔ editing JSON             | native 形式自体が JSON。KCES2 のみ。同名の `.undresspdat` と対で使用                                                                                                                                                    |
| `.undresspdat`                                  | 下着めくり事前計算キャッシュ                 | native ↔ editing JSON             | native 形式自体が JSON。KCES2 のみ。同名の `.undressdat` から生成                                                                                                                                                       |
...
| native Texture2D、Sprite object file            | 画像                                         | Texture2D → PNG/DDS、Sprite → PNG | 一方向変換。Sprite 自体はピクセルを持たないため、見た目を変えるには参照先の Texture2D を編集します。[Sprite の編集](docs/cli-document.md#sprite-の編集)を参照                                                           |
...
[完全な CLI ドキュメント](docs/cli-document.md)も参照してください。
...
[docs/cli-document.md](docs/cli-document.md) にあります。`--strict`（`-s`）は内容に基づく形式判定を有効にし、`--type`（`-t`）は native type
