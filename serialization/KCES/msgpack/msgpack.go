...
func CompressLz4BlockArray(data []byte) ([]byte, error) {
	...
	numBlocks := (dataLength-1)/blockSize + 1
	...
	for offset := int64(0); offset < dataLength; offset += blockSize {
		end := min(offset+blockSize, dataLength)
...
func encodeLz4LiteralBlock(block []byte) []byte {
	literalLength := int64(len(block))
	var extraLengthBytes int64
	if literalLength >= 15 {
