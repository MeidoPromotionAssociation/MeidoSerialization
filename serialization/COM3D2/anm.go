...
func ReadAnm(r io.Reader) (*Anm, error) {
	...
	for {
		...
		case b == 1:
			...
			currentBoneIndex = int64(len(clip.BoneCurves)) - 1
		...
		case b >= 100:
			...
			for i := int32(0); i < keyframeCount; i++ {
...
func (a Anm) Dump(w io.Writer) error {
	...
	for _, boneData := range a.BoneCurves {
		...
		for _, pc := range boneData.PropertyCurves {
			...
			// keyframe 数量
			// Keyframe count
			kfCount, err := collectionCountInt32("animation keyframe count", int64(len(pc.Keyframes)))
			if err != nil {
				return err
			}
...
func validateAnmForDump(animation *Anm) error {
	...
	for boneIndex := range animation.BoneCurves {
		...
		for propertyIndex := range bone.PropertyCurves {
			...
			if _, err := collectionCountInt32(fmt.Sprintf("BoneCurves[%d].PropertyCurves[%d] keyframe count", boneIndex, propertyIndex), int64(len(property.Keyframes))); err != nil {
				return err
			}
		}
	}
