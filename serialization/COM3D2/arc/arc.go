...
func ReadArc(rs io.ReadSeeker) (*Arc, error) {
	...
	header, err := reader.ReadBytes(int64(len(arcHeader)))
	if err != nil {
		return nil, err
	}
	...
	for utf8HashData == nil || utf16HashData == nil || utf16NameData == nil {
		...
		// UTF-16 hash table
		case 0:
			if utf16HashData, err = reader.ReadBytes(blockSize); err != nil {
				return nil, fmt.Errorf("failed to read utf16 hash data: %w", err)
			}
		// UTF-8 哈希表
		// UTF-8 hash table
		case 1:
			if utf8HashData, err = reader.ReadBytes(blockSize); err != nil {
		...
		case 3:
			...
			data, err := reader.ReadBytes(int64(compressedSize))
			if err != nil {
	...
	if rootName, ok := nameLUT[utf16HT.ID]; ok {
		...
		base := rootName
		if i := lastIndexOfSep(rootName); i >= 0 && i+1 < int64(len(rootName)) {
			base = rootName[i+1:]
		}
		arc = NewArc(base)
	}
...
func (arc *Arc) Dump(path string) error {
	...
	for i, fl := range files {
		...
		rawSize, err := checkedArcUint32Length(fmt.Sprintf("file %q raw size", fl.RelativePath()), int64(len(raw)))
		if err != nil {
			return err
		}
		storedSize, err := checkedArcUint32Length(fmt.Sprintf("file %q stored size", fl.RelativePath()), int64(len(enc)))
		if err != nil {
			return err
		}
		pos, err := writer.Tell()
	...
	nameRawSize, err := checkedArcUint32Length("UTF16 name table raw size", int64(len(nameRaw)))
	if err != nil {
		return err
	}
	nameStoredSize, err := checkedArcUint32Length("UTF16 name table compressed size", int64(len(nameEnc)))
	if err != nil {
		return err
	}
...
func checkedArcUint32Length(path string, length int64) (uint32, error) {
...
func checkedArcInt32Count(path string, count int64) (int32, error) {
...
func (arc *Arc) calculateDirOffsets(uuidToHash map[uint64]uint64) map[uint64]int64 {
	...
	rec = func(d *Dir) {
		...
		cnt := int64(len(d.Dirs)) + int64(len(d.Files))
		offset += 16 * cnt
		// 每个父目录哈希占 8 字节
