...
func ReadMorphData(reader *stream.BinaryReader, version int32) (*MorphData, error) {
	...
	md.Indices = makeCountedSliceForAppend[int32](vertCount)
	...
	for i := int32(0); i < vertCount; i++ {
		...
		md.Indices = append(md.Indices, int32(index))
...
func validateModelForDump(model *Model) error {
	...
	if _, err := collectionCountInt32("model bone count", int64(len(model.Bones))); err != nil {
		return err
	}
	if _, err := collectionCountInt32("model vertex count", int64(len(model.Vertices))); err != nil {
		return err
	}
	if _, err := collectionCountInt32("model submesh count", int64(len(model.SubMeshes))); err != nil {
		return err
	}
	if _, err := collectionCountInt32("model mesh bone count", int64(len(model.BoneNames))); err != nil {
		return err
	}
	...
	if _, err := collectionCountInt32("model material count", int64(len(model.Materials))); err != nil {
		return err
	}
	if _, err := collectionCountInt32("model tangent count", int64(len(model.Tangents))); err != nil {
		return err
	}
	...
	for submeshIndex, submesh := range model.SubMeshes {
		if _, err := collectionCountInt32(fmt.Sprintf("model SubMeshes[%d] triangle count", submeshIndex), int64(len(submesh))); err != nil {
			return err
		}
...
func validateMorphDataForDump(path string, morph *MorphData, version int32) error {
	...
	count, err := collectionCountInt32(path+" vertex count", int64(len(morph.Indices)))
	if err != nil {
		return err
	}
...
func validateSkinThicknessForDump(thickness *SkinThickness) error {
	...
	if _, err := collectionCountInt32("skin thickness group count", int64(len(thickness.Groups))); err != nil {
		return err
	}
	for _, key := range keys {
		group := thickness.Groups[key]
		if _, err := collectionCountInt32(fmt.Sprintf("skin thickness group %q point count", key), int64(len(group.Points))); err != nil {
			return err
		}
		for pointIndex, point := range group.Points {
			...
			if _, err := collectionCountInt32(fmt.Sprintf("skin thickness group %q Points[%d] angle count", key, pointIndex), int64(len(point.DistanceParAngle))); err != nil {
				return err
			}
