...
func ReadNei(r io.Reader, neiKey []byte) (*Nei, error) {
	...
	for i := int64(0); i < totalCells; i++ {
		...
		if length > 0 {
			...
			cellData := stringData[offset:end]
...
func (nei *Nei) Dump(w io.Writer) error {
	...
	for rowIndex := int64(0); rowIndex < int64(len(nei.Data)); rowIndex++ {
		row := nei.Data[rowIndex]
		for colIndex := int64(0); colIndex < int64(nei.Cols); colIndex++ {
			cellIndex := colIndex + rowIndex*int64(nei.Cols)
...
func checkedNEICellCount(rows, cols uint32) (int64, error) {
...
func validateNEIShape(nei *Nei) (int64, error) {
	if nei == nil {
