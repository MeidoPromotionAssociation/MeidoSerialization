...
func kcesContainerProfiles() map[string]Guide {
	...
	ctWidthSource := source("KCES2 1.36.0", "KCES2 1.36.0/WfSystem.FileSystem/Catalog/AssetBundleCatalog.cs", "AssetBundleCatalog.resourceFileBuildAssetBundleNames/contentHash", 216, 257, "AssetBundleCatalog declares [Key(0)] through [Key(13)] with FixVersion 1001, where resourceFileBuildAssetBundleNames and contentHash are the two trailing keys absent from the twelve-slot version 1000 layout.")
	ctSharedBundleSource := source("KCES2 1.36.0", "KCES2 1.36.0/WfSystem.FileSystem/ResourceManagement/InitializationOperation.cs", "InitializationOperation.Execute/SharedAssetBundleResourceRegistry", 118, 278, "Catalog initialization reads GetResourceFileBuildAssetBundleName for each resource index and reuses an already registered AssetBundleResourcePack under that case-insensitive name, while a null or empty name is neither looked up nor registered.")
	...
	ct := guide(
		...
		FormatVerificationSerializationVerified,
		"AssetBundleCatalog, CatalogUtility, and the content-table reader were reviewed in KCES 1.34.4, and the version 1001 catalog keys plus their AssetBundle sharing path were reviewed in KCES2 1.36.0. Catalog hashes and index relationships are runtime-sensitive; only independently named side files outside the catalog declarations remain byte payloads.",
		[]Source{ctSource, ctWidthSource, ctSharedBundleSource, ctUtilitySource, vdSource},
	...
	ct.FieldPatterns = []FieldPattern{
		...
		pattern("/catalog/resourceFileBuildAssetBundleNames", "Shared AssetBundle names", "The build-time AssetBundle name for each resource index, present only in the version 1001 catalog layout.", "Catalog initialization reuses one loaded AssetBundleResourcePack for every catalog that reports the same case-insensitive name and skips both lookup and registration when the name is null or empty.", "runtime_catalog", "Keep one entry per resourceFileNames index and reuse the exact name only for resources that really are the same built AssetBundle; leave the field null to keep a resource unshared.", ctWidthSource, ctSharedBundleSource),
		pattern("/catalog/contentHash", "Catalog content hash", "A 64-character hexadecimal content hash present only in the version 1001 catalog layout.", "No reviewed runtime path reads this member; the catalog only stores and reloads it.", "runtime_metadata", "Preserve the decoded value and leave it null when the source catalog has none; there is no reviewed way to recompute it.", ctWidthSource),
		pattern("/catalog/indexedArrayWidth", "Catalog wire width", "The recorded AssetBundleCatalog indexed-array width, 12 for the version 1000 layout and 14 for the version 1001 layout.", "It is not a game member; the game reads whichever width the file stores and leaves absent trailing keys at their constructor defaults.", "editing_metadata", "Preserve the decoded width; use 14 when resourceFileBuildAssetBundleNames or contentHash must be written and 12 or omitted otherwise.", ctWidthSource),
	...
	ct.Rules = []Rule{
		{ID: "catalog-reference-coherence", AppliesTo: []string{"/catalog", "/extensionNameLists"}, Severity: "error", Summary: "Catalog hashes and indices must remain coherent.", Details: "A catalog item points to resourceFileNames by resourceIndex and is found by its hash; extension-name lists repeat the same name/hash relationship. Update all copies together.", Evidence: []Source{ctSource, ctUtilitySource}},
		{ID: "catalog-version-1001-width", AppliesTo: []string{"/catalog/indexedArrayWidth", "/catalog/resourceFileBuildAssetBundleNames", "/catalog/contentHash"}, Severity: "error", Summary: "The two version 1001 catalog keys exist only in the fourteen-slot layout.", Details: "An AssetBundleCatalog stores either twelve slots ending at items or fourteen slots ending at contentHash, and the stored version is 1000 or 1001 accordingly. resourceFileBuildAssetBundleNames and contentHash therefore require indexedArrayWidth 14; writing them into the twelve-slot layout is rejected because the game would never find those keys.", Evidence: []Source{ctWidthSource}},
	}
	ct.Invariants = []string{"catalog is required for a valid .ct envelope.", "resourceIndex values point into resourceFileNames.", "Hash fields use the game's case-insensitive resource-name hash convention.", "An AssetBundleCatalog stores 12 slots for version 1000 or 14 slots for version 1001, and a VirtualAssetCatalog always stores 10."}
