...
func kcesProfiles() map[string]Guide {
	...
	nson := guide(
		...
		FormatVerificationSerializationVerified,
		"The KCES loader entry point and semantic JSON boundary were reviewed. The editing form is the parsed JSON document itself and deliberately normalizes BOM, whitespace, and formatting; domain fields vary by resource.",
		[]Source{jsonSource},
		[]Field{
			field("/", "Normalized .nson document", "The whole parsed JSON document, which is the editing JSON root itself.", "CsvManager supplies JSON text to the consuming resource code; whitespace and BOM have no independent game meaning and this library does not infer the domain schema.", "opaque_domain_data", "Change only keys documented by the specific resource or game feature; retain unknown semantic members and types.", "critical"),
		},
	)
